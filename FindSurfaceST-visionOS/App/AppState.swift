...
final class AppState {
    ...
    private(set) var faces: [UUID: [simd_uint3]] = [:]
    ...
    func generateVisiblePoints(_ filteringID: UUID? = nil, _ filteringIndex: Int? = nil) -> [simd_float3] {
        
        guard let deviceAnchor else { return [] }
        
        guard let filteringID,
              let filteringIndex else {
            return visiblePoints
        }
        
        var stopTracking = false
        let filteredPoint = pointclouds[filteringID]![filteringIndex]
        let points = pointclouds.flatMap { (id, pointcloud) in
            if !stopTracking && id == filteringID {
                stopTracking = true
                return pointcloud.enumerated().filter { (index, _) in
        ...
        return [filteredPoint] + results
    }
    
    func pickPoint(near location: simd_float3) -> Int? {
    ...
    func pickPoint(near location: simd_float3, entity: Entity) -> Int? {
        
        guard let deviceAnchor else { return nil }
        
        
        guard let entityID = UUID(uuidString: entity.name),
              let points = pointclouds[entityID],
              let face = faces[entityID] else {
            return pickPoint(near: location)
        }
        
        let rayOrigin = deviceAnchor.position
        let rayDirection = normalize(location - rayOrigin)
        
        let hitSequence = face.lazy.compactMap { faceIndices -> (simd_float3, Int, Int, Int, Float)? in

            let i0 = Int(faceIndices.x)
            let i1 = Int(faceIndices.y)
            let i2 = Int(faceIndices.z)
            ...
            let normal = normalize(cross(normalize(v1 - v0), normalize(v2 - v0)))
            
            guard let hit = hitPlane(rayOrigin, rayDirection, center, normal) else {
                return nil
            }
            
            let bc = barycentricCoordinate(v0, v1, v2, hit)
            let range: ClosedRange<Float> = 0...1
            guard range.contains(bc.x) && range.contains(bc.y) && range.contains(bc.z) else { return nil }
            
            return (hit, i0, i1, i2, simd_distance_squared(hit, location))
        }
        
        let nearestHit = hitSequence.min { (lhs, rhs) in
            lhs.4 < rhs.4
        }
        
        guard let (hitLocation, i0, i1, i2, _) = nearestHit else { return pickPoint(near: location) }
        
        let v0 = points[i0]
        let v1 = points[i1]
        let v2 = points[i2]
        
        let d0 = normalize(v0 - hitLocation)
        let d1 = normalize(v1 - hitLocation)
        let d2 = normalize(v2 - hitLocation)
        
        let d3 = normalize(hitLocation - rayOrigin)
        
        let d03 = abs(dot(d0, d3))
        let d13 = abs(dot(d1, d3))
        let d23 = abs(dot(d2, d3))
        
        switch min(d03, min(d13, d23)) {
        case d03: return i0
        case d13: return i1
        case d23: return i2
        default: return nil
    ...
    func updateFaces(_ faces: [simd_uint3]?, forKey key: UUID) {
        guard let faces else {
            self.faces.removeValue(forKey: key)
            return
        }
        
        self.faces.updateValue(faces, forKey: key)
    }
    ...
    private func anchorAdded(_ anchor: MeshAnchor) async {
        ...
        updateFaces(anchor.faces, forKey: anchor.id)
    }
    ...
    private func anchorRemoved(_ anchor: MeshAnchor) async {
        ...
        updateFaces(nil, forKey: anchor.id)
    }
